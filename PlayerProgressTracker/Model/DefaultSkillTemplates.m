...
static float toughnessBasicBarrier = 3.4;
static float toughnessProgression = 3.4;

static float mentalSkillHightBasicBarrier = 4.2;
static float mentalSkillHightProgression = 4.2;
    ...
    if (!_physique){
        ...
        skillTemplate = [self newSkillTemplateWithUniqName:physiqueKey
                                            nameForDisplay:physiqueName
                                                 withRules:[NSString stringWithFormat:@"Rules:\n- Physique determines how far character can move per round. Character can sprint up to a double of normal movement, but in cost of Endurance. Every time character sprints - he loses 1 Endurance point and comes closer to becoming tired.\n- Character get 1 xp point to a certain skill, whenever he uses this skill. Character will get 1 xp point for check, no matter was that check successful or not. Success or Fail describe whenever character is progress thought the game.\n - Any weapon skills usually doesn't require checks and give static bonus. Character get experience bonus whenever he choose to fight. In a battle every turn player fight - he get xp point, according to the weapon or battle style he uses.\n- Training skill outside real-time mode (like saying that “my character swing a sword for 2 hours”) will give him 1 xp point per 3 hours. This rule works unless skill description or rules notice otherwise. Craft, for example, give 1 xp point per day (%d hours). Skill that require check against opponent skill will require someone to participate.",dayLastHours]
    ...
    if (!_intelligence){
        ...
        skillTemplate = [self newSkillTemplateWithUniqName:intelligenceKey
                                            nameForDisplay:intelligenceName
                                                 withRules:[NSString stringWithFormat:@"Rules:\n- Character get 1 xp point to a certain skill, whenever he uses this skill. Character will get 1 xp point for check, no matter was that check successful or not. Success or Fail describe whenever character is progress thought the game.\n- Training skill outside real-time mode will give him 1 xp point per 3 hours. This rule works unless skill description or rules notice otherwise. Craft, for example, give 1 xp point per day (%d hours). Skill that require check against opponent skill will require someone to participate.",dayLastHours]
                                         withRulesExamples:nil
    ...
    if (!_strength){
        ...
        skillTemplate = [self newSkillTemplateWithUniqName:strengthKey
                                            nameForDisplay:strengthName
                                                 withRules:[NSString stringWithFormat:@"Rules:\n- All melee damage get bonus from Strength.\n- This skill opposes enemy Toughness. Before damage is dealt - it lowers by Toughness.\n- Inventory. Any item you pick has Encumbrance value. This value represents difficulty in bearing item around. Some can be very heavy, others - light yet too long to support free movement. The stronger your character - more stuff can he keep around without getting overloaded. Overall character carrying capacity equal to level of this skill  multiplied by 2. If overloaded character gain 1 penalty for Agility and Strength related skill checks for every %d point of excess encumbrance, unless noted otherwise. In addition character can't run and moving at full speed will cost Endurance points. If Agility or Strength penalties reach skill level - character can't move or act carrying so much stuff around. \nNote: some creatures will have Encumbrance points by default, representing their own mass and how it is affecting their mobility.\n- Character gain adrenalin whenever he understand the danger to his life in current situation. Character get 3 adrenalin points each time and is able to accumulate up to his Strength adrenaline points.\n- Adrenalin is always given every round during the battle, if of cause character aware the battle is going on. If character decide to flee or in any way avoid the confrontation he still get adrenalin points.\n- Outside direct confrontation, adrenalin is gained only if character is under direct threat (or if he feels that his personal boundaries are violated) and only with some stress points. If someone decide to cause himself pain in order to invoke adrenalin, pain should be sever enough to cause at least 1 stress point.\n- Each adrenalin point give +1 to all Physique related skill checks.\n- You loose 1 Adrenalin point (as well as Fear or Fatigue acquired Stress point) each %d minutes, in case nothing treating your live.",defaultEncumbrancePenalties ,minutesToLooseStress]
                                         withRulesExamples:nil
    ...
    if (!_toughness){
        ...
        skillTemplate = [self newSkillTemplateWithUniqName:toughnessKey
                                            nameForDisplay:toughnessName
                                                 withRules:@"Rules:\n- Character hit points equal to double the number of Toughness(To) levels plus 4 points for each point of Bulk (humans have 1 bulk).\n- In any Toughness related check, where To is used to represent mass of the creature - multiply To by Bulk. Massive monsters will need a lot more Strength to knock down or they will deal much more damage in trampling someone. Yet any disease will affect them like any creature.\n- Toughness is a natural armor. Every time character obtain damage - it is lowered by Toughness level.\n- Toughness determines number of rounds character can hold his breath or bear hostile environment without dying.\n- Toughness determines number of rounds character can perform physical activity in a row without becoming tired. In combat-like environment, where time divided by rounds, character spend Endurance when:\n\n• Spend all action points, during round.\n• Sprint.\n• Trying to make any non auto-success skill checks.\n\nIf character is overloaded:\n• Picking any action during battle.\n• Moving with full speed. Sprint is disable.\n• Trying to make any non auto-success skill checks.\n\n\nAfter character spent all his Endurance points - for each round of hard work he gain 1 stress point. Every stress point add penalty -1 modifier to all skill checks. Melee weapon action points modify accordingly.\n- Resting (or sleeping and eating) restores 10% hit point and 10% mental point every 8 hours. Resting require non-stressful, safe place to sleep and at least 1 portion of food. Resting in a bed restores 20% mental points. Spending 3 portion of food restores 20% points of health. You can rest only once per 24 hours.\n- Nightmares won't let restore mental points during sleeping.\n- Not sleeping or not eating at all doesn't mean your character don't eat or sleep. He finds a little food and little time to rest, but only enough to keep going."
                                         withRulesExamples:@"d6 checks against modifier. Hostile environment. Every failed checks result in stress points.\n- Modifier -1. Character affected by environment with a bit different climate. (~10+ C) Checks made every two hours.\n- Modifier -3. Character affected by environment with noticeably different climate. (~20+ C) Checks made every hour.\n- Modifier -5. Character affected by environment with much different climate. (~30+ C)  Checks made every half-hour.\n- Modifier -7. Character affected by environment with extremely different climate. (~40+ C) Checks made every minute.\n\nd6 checks against modifier. Endurance in performing hard work.\n- Modifier -2. Perform exhausting activity for 5 rounds or half of the minute.\n- Modifier -4. Perform exhausting activity for 10 rounds or 1 minute. etc. -2 for every 5 rounds."
                                           withDescription:@"Toughness opposes opponent Strength during battle and number of modifiers against illness or poison effects. In some cases Toughness can be used as a weapon to crash the target."
...
-(SkillTemplate *)fly{
    if (!_fly){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:flyKey
                                            nameForDisplay:flyName
                                                 withRules:@"Rules:\n- You generally need only make a Fly check when you are attempting a complex maneuver. Without making a check, a flying creature can remain flying at the end of its turn so long as it moves a distance greater than half its speed. It can also turn up to 45 degrees by sacrificing 5 feet of movement, can rise at half speed at an angle of 45 degrees, and can descend at any angle at normal speed. Note that these restrictions only apply to movement taken during your current turn. At the beginning of the next turn, you can move in a different direction than you did the previous turn without making a check. Taking any action that violates these rules requires a Fly check. The difficulty of these maneuvers varies depending upon the maneuver you are attempting, as noted on the following chart.\n- Since the game is generally assumed to be played in two dimensions, even when representing three dimensional combat, the rules for ascending are handled by the speed reduction instead of asking players and GMs to ascertain the diagonal vertical movement.\n- Attacked While Flying. If you take hit while flying, you must make a check to avoid losing altitude. This descent does not count against a creature’s movement.\n- Collision While Flying. If you are using wings to fly and you collide with an object equal to your size or larger, you must immediately make Fly check to avoid plummeting to the ground, taking the appropriate falling damage.\n- Avoid Falling Damage. If you can fly, you can use Fly skill to make falling checks, instead of Agility.\n- High Wind Speeds. Flying in high winds adds penalties on your Fly checks as noted on the Wind Effects table.\n- If you are using wings and you fail a Fly check by 2-3 or auto-fail, you plummet to the ground, taking the appropriate falling damage, else you can attempt to perform the same maneuver on subsequent rounds."
                                         withRulesExamples:@"Additionally all fly checks affected by the wind:\n- Modifier -2. Strong wind. 21-30 mph. 32-48kmh.\n- Modifier -4. Severe. 31-50 mph. 49-80 kmh.\n- Modifier -6. Windstorm. 51-74 mph. 81-120 kmh.\n- Modifier -8. Hurricane. 75-174 mph. 121-281 kmh.\n- Modifier -10. Tornado. 175+ mph. 282kmh.\n\nd6 checks against modifier. Flying.\n- Modifier -2. per point of Bulk.\n- Modifier 0. Move less than half speed and remain flying\n- Modifier -2. Hover. Turn greater than 45˚ by spending 5 points of movement\n- Modifier -4. Turn 180˚ by spending 10 points of movement. Fly up at a greater than 45˚ angle.\n- Modifier -6. Avoid Collision.\n\nd6 checks against Hit.\n- Modifier “Hit Damage“. \nFail: Loose 5 points of altitude.\nAuto-Fail: Loose 5 plus difference between you Flying skill and “Hit Damage“ points of altitude."
                                           withDescription:@"You are skilled at flying, either through the use of wings or magic, and you can perform daring or complex maneuvers while airborne. Note that this skill does not give you the ability to fly.\nHere instead of meters or feets used term “points”. Read as main system prescribe what metrics should be used."
                                             withSkillIcon:@"fly"
                                        withBasicXpBarrier:defaultAdvBasicBarrierLow
                                      withSkillProgression:defaultAdvProgression
                                  withBasicSkillGrowthGoes:defaultAdvGrowhtGoes
    ...
    if (!_senseMotive){
        ...
        skillTemplate = [self newSkillTemplateWithUniqName:senseMotiveKey
                                            nameForDisplay:senseMotiveName
                                                 withRules:@"Rules:\n- Hunch. You can get the feeling from another's behavior that something is wrong, such as when you're talking to an impostor. Alternatively, you can get the feeling that someone is trustworthy.\n- Recognize care. Recognize when someone doing good for thing. Allow additional bonus points from performing virtue. \n- Sense Enchantment. If haveMagic skill bellow 8 you gain -4 penalty. You can tell that someone's behavior is being influenced by an enchantment effect even if that person isn't aware of it.\n- Discern Secret Message. You may use Sense Motive to detect that a hidden message is being transmitted via the Bluff skill. In this case, your Sense Motive check is opposed by the Bluff check of the character transmitting the message. For each piece of information relating to the message that you are missing, you take a -2 penalty on your Sense Motive check."
                                         withRulesExamples:@"d6 check against opponent Control. Hunch.\nd6 check against opponent Ritual Magic. Sense Enchantment.\nd8 check against opponent Bluff. Discern Secret Message.Sense Enchantment.\n\nOn any Virtue make d6 Sense Motive check against according modifier from reputation table:\n- On success restore 1 morality point and increase reputation towards the subject by 1 point.  \n- On auto-success bonus to morality and reputation additionally increase on those extra points.\n- On fail or auto-fail no changes occurs."
    ...
    if (!_magic){
        ...
        skillTemplate = [self newSkillTemplateWithUniqName:magicKey
                                            nameForDisplay:magicName
                                                 withRules:@"Rules: \n- To successfully cast spell make d6 check against modifier. Complexity of the spell represented in modifier. You free to increase the modifier, boosting spell power, according to the spell description.\nSuccess: spell casted as intended.\nFail: spell casted, but it power lowers accordingly to failing points on the dice. So, if you failed check only point from success - spell loses 1 point of spellpower.\nAuto-Fail: spell won't work.\n- Failing spell check with 2 on d6 will cause magic mistake. Roll Magic mischief table to know the result.\n- Any wizard within area with radius equal to his/her Magic level can sense magic being casted and can influence the result. Wizard can add d6 points to spell modifier in hope to achieve Auto-Fail. After d6 rolled add additional points and make d6 Control check against number on the roll. Wizard capable to add any number of d6 to spell modifier, but each time Control check will follow."
                                         withRulesExamples:@"d6 check against modifier. Cast Spell."
