...
static float craftBasicBarrier = 3;
static float craftProgression = 2;
...
-(SkillTemplate *)physique{
    if (!_physique){
        ...
        skillTemplate = [self newSkillTemplateWithUniqName:physiqueName
                                                 withRules:nil
...
-(SkillTemplate *)intelligence{
    if (!_intelligence){
        ...
        skillTemplate = [self newSkillTemplateWithUniqName:intelligenceName
                                                 withRules:nil
                                         withRulesExamples:nil
...
-(SkillTemplate *)melee{
    if (!_melee){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:weaponSkillName
                                                 withRules:nil
                                         withRulesExamples:nil
...
-(SkillTemplate *)range{
    if (!_range){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:ballisticSkillName
                                                 withRules:nil
                                         withRulesExamples:nil
...
-(SkillTemplate *)magic{
    if (!_magic){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:magicName
...
-(SkillTemplate *)strength{
    if (!_strength){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:strengthName
...
-(SkillTemplate *)toughness{
    if (!_toughness){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:toughnessName
                                                 withRules:nil
                                         withRulesExamples:nil
...
-(SkillTemplate *)agility{
    if (!_agility){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:agilityName
...
-(SkillTemplate *)reason{
    if (!_reason){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:reasonName
                                                 withRules:nil
                                         withRulesExamples:nil
...
-(SkillTemplate *)control{
    if (!_control){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:disciplineName
                                                 withRules:nil
                                         withRulesExamples:nil
...
-(SkillTemplate *)perception{
    if (!_perception){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:perceptionName
...
-(SkillTemplate *)climb{
    if (!_climb){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:climbName
                                                 withRules:[NSString stringWithFormat:@"Rules:\n- Character climbing speed equals to the skill minus level of the task. Minimum speed is equal to character Physique level.\n- To climb an obstacle perform Climb check against level of the obstacle. If character fail you start slipping or fall down (depending on a type of obstacle: climbing a pillar character will slip, but ceiling with handholds - will fall down). If climb skill check drops below zero you automatically fall down (even if character would normally slip).\n- In addition for every %d points of encumbrance (counting equipment you are wearing) your climb level decreased by 1.\n- To perform any other physique related actions you need to pass climb test agaist level of the task.",defaultEncumbrancePenalties]
                                         withRulesExamples:@"d6 Climb checks:\n- Modifier -2. A slope too steep to walk up, or a knotted rope with a wall to brace against, ladder. Or living obstacle which move with speed of 1-2 or fighting in close combat.\n- Modifier -4. A surface with ledges to hold on to and stand on, such as a very rough wall or a ship’s rigging. Any surface with adequate handholds and footholds (natural or artificial), such as a very rough natural rock surface or a tree, or an unknotted rope, or pulling yourself up when dangling by your hands. Or living obstacle which move with speed of 3-5.\n- Modifier -6. An uneven surface with some narrow handholds and footholds, such as a typical wall in a dungeon or ruins. Or living obstacle which move with speed of 6-9.\n- Modifier -8. A rough surface, such as a natural rock wall or a brick wall. Or an overhang or ceiling with handholds but no footholds. Or living obstacle which move with speed of 10 and more."
...
-(SkillTemplate *)stealth{
    if (!_stealth){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:stealthName
                                         ...
                                         withRulesExamples:@"d6 Stealth checks:\n- Modifier 0. Normal surface/Dark room.\n- Modifier -5. Noisy surface(scree, shallow or deep bog, undergrowth, dense rubble)/Poorly-lit room.\n- Modifier -10. Very noisy(dense undergrowth, deep snow)/Well-lit room"
...
-(SkillTemplate *)escapeArtist{
    if (!_escapeArtist){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:escapeArtistName
...
-(SkillTemplate *)knavery{
    if (!_knavery){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:knaveryName
                                         ...
                                         withRulesExamples:@"d6 Knavery check:\n- Modifier -2. Palm a coin-sized object, make a coin disappear.\n- Modifier -4. Lift a small object from a person.\n- Modifier -7. Lift a sheathed weapon from another creature and hide it on the character’s person, if the weapon is no more than one size category larger than the character’s own size.\n- Modifier -10. Make an adjacent, willing creature or object of the character’s size or smaller “disappear” while in plain view. In fact, the willing creature or object is displaced up to 10 feet away—make a separate knavery test to determine how well the “disappeared” creature or object is hidden."
...
-(SkillTemplate *)senseMotive{
    if (!_senseMotive){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:senseMotiveName
...
-(SkillTemplate *)disguise{
    if (!_disguise){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:disguiseName
...
-(SkillTemplate *)appraise{
    if (!_appraise){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:appraiseName
                                         ...
                                         withRulesExamples:@"d6 test against rarity level of item. On success get item price. On fail price modified accordingly to value of the the dice multiplied by item rarity level. "
...
-(SkillTemplate *)hackDevice{
    if (!_hackDevice){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:hackDeviceName
...
-(SkillTemplate *)heal{
    if (!_heal){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:healName
                                         ...
                                         withRulesExamples:@"d8 check. Treat Poison. Treat Disease. Treat Injuries. Long-Term Care..."
...
-(SkillTemplate *)craft{
    if (!_craft){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:craftName
                                                 withRules:@"Rules:\n- You can repair an item by making checks against the same difficulty level that it took to make the item in the first place. The cost of repairing an item is one-fifth of the item's price (if isn't noticed otherwise).\n- Craft checks are made by the day.\n- You can try again, but if complexity of the item is enough to miss with 2-4 on d6, you ruin half the raw materials and have to pay half the original raw material cost again.\n- Accelerated Crafting. You can get some help, but others must have enough craft skill to have a chance to create item with difficulty decreased by 3 points, speed increases by the number of people working on a project. Each make separate check. Helpers make the same check with decreased difficulty (by 6)."
                                         withRulesExamples:@"d6 check against Complexity of the item. Craft or repair the item."
                                           withDescription:@"Advanced skill. You are skilled in the creation of a specific group of items, such as armor or weapons. You know how to use the tools of your trade, how to perform the craft's daily tasks, how to supervise untrained helpers, and how to handle common problems. (Untrained laborers and assistants earn an average of 4 coins day.)"
                                             withSkillIcon:nil
                                        withBasicXpBarrier:craftBasicBarrier
                                      withSkillProgression:craftProgression
                                  withBasicSkillGrowthGoes:defaultAdvGrowhtGoesLow
                                             withSkillType:AdvancedSkillType
                                    withDefaultStartingLvl:0
                                   withParentSkillTemplate:self.reason
...
-(SkillTemplate *)animalHandling{
    if (!_animalHandling){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:animalHandlingName
...
-(SkillTemplate *)diplomacy{
    if (!_diplomacy){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:diplomacyName
...
-(SkillTemplate *)intimidate{
    if (!_intimidate){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:intimidateName
...
-(SkillTemplate *)crashing{
    if (!_crashing){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:bluntName
                                                 withRules:nil
                                         withRulesExamples:nil
...
-(SkillTemplate *)cutting{
    if (!_cutting){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:ordinaryName
                                                 withRules:nil
                                         withRulesExamples:nil
...
-(SkillTemplate *)piercing{
    if (!_piercing){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:flailName
                                                 withRules:nil
                                         withRulesExamples:nil
...
-(SkillTemplate *)bow{
    if (!_bow){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:bowName
                                                 withRules:nil
                                         withRulesExamples:nil
...
-(SkillTemplate *)firearm{
    if (!_firearm){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:blackpowderName
                                                 withRules:nil
                                         withRulesExamples:nil
...
-(SkillTemplate *)thrown{
    if (!_thrown){
        SkillTemplate *skillTemplate;
        skillTemplate = [self newSkillTemplateWithUniqName:thrownName
                                                 withRules:nil
                                         withRulesExamples:nil
