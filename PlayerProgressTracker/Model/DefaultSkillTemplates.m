...
//second level skills
static float defaultPhsAndMnsProgression = 4;
static float defaultPhsAndMnsBasicBarrier = 4;
static float defaultPhsAndMnsGrowhtGoes = 0.14;

static float toughnessBasicBarrier = 3.2;
static float toughnessProgression = 3.2;

static float perceptionBasicBarrier = 4.8;
...
static float defaultRangeWeaponProgressionFirearm = 3;
static float defaultRangeWeaponBasicBarrierFirearm = 5;
    ...
    if (!_agility){
        ...
        if (!existingSkillsTemplateWithThisName || existingSkillsTemplateWithThisName.count==0 || self.shouldUpdate){
            skillTemplate = [SkillTemplate newSkillTemplateWithUniqName:agilityName
                                                              withRules:[NSString stringWithFormat:@"Rules: \n- For jumping, traversing narrow or treacherous surfaces pass the Agility test*.\n- For every %d points of encumbrance (counting equipment you are wearing) your agility level decreased by 1.\n- When falling down from less then 6 meters you can attempt to land on feet taking same damage with 0 Strength.\n- Using a balancing pole (two hands) while traversing a narrow surface grants +1 to Agility check.\n- If you use a pole as part of a running jump, you gain a +2 bonus on your Agility check (but must let go of the pole in the process).",defaultEncumbrancePenalties]
                                                      withRulesExamples:@"*Here are examples of tasks and their stats:\n- d6 test. Move on narrow surface or uneven ground without falling at half speed. Running Jump with distance equal to the half of your movement. Running distance must be more or equal to jumping distance. Land on a feet falling from 1 meter height.\n- d8 test. Jump from a place with distance equal to the half of your movement. Land on a feet falling from 2 meter height.\n- d10 test. Running Jump with distance equal to your movement. Running distance must be more or equal to jumping distance. Land on a feet falling from 3 meter height.\n- d12 test. Move(at full speed)/Fight on narrow surface or uneven ground without falling. Land on a feet falling from 4 meter height.\n- d20 test. Jump from a place with distance equal to your movement. Land on a feet falling from 5 meter height."
    ...
    if (!_control){
        ...
        if (!existingSkillsTemplateWithThisName || existingSkillsTemplateWithThisName.count==0 || self.shouldUpdate){
            skillTemplate = [SkillTemplate newSkillTemplateWithUniqName:disciplineName
                                                              withRules:nil
                                                      withRulesExamples:nil
                                                        withDescription:@"Advanced skill. Resist charm, resist guile, resist intimidation, resist fear, resist terror, resist torture. This skill is used to resist the startling effects of surprising events, show resolve in the face of danger, and maintain composure when confronted by supernatural or terrify- ing situations. Discipline is also the ability to maintain one’s state of mind and resist the rigours of stress or attempts to manipulate one’s thoughts or feelings."
                                                          withSkillIcon:nil
                                                     withBasicXpBarrier:defaultPhsAndMnsBasicBarrier
                                                   withSkillProgression:defaultPhsAndMnsProgression
                                               withBasicSkillGrowthGoes:defaultPhsAndMnsGrowhtGoes
        ...
        if (!existingSkillsTemplateWithThisName || existingSkillsTemplateWithThisName.count==0 || self.shouldUpdate){
            skillTemplate = [SkillTemplate newSkillTemplateWithUniqName:perceptionName
                                                      ...
                                                      withRulesExamples:@"*Here are examples of surroundings and their stats:\nLevel 1. d10 test. Normal well-lit place. \nLevel 2. 2d8 test. Poorly-lit place. \nLevel 3. 2d12 test. Dark place."
                                                        withDescription:@"Advanced skill. Eavesdropping, tracking, keen vision, minute details Using your senses to perceive your surroundings. It can also be used to spot traps, pitfalls, and other physical dangers. Perception opposes other characters’ attempts at Stealth, or to otherwise avoid detection.."
                                                          withSkillIcon:nil
                                                     withBasicXpBarrier:perceptionBasicBarrier
    ...
    if (!_swimming){
        ...
        if (!existingSkillsTemplateWithThisName || existingSkillsTemplateWithThisName.count==0 || self.shouldUpdate){
            skillTemplate = [SkillTemplate newSkillTemplateWithUniqName:swimmingName
                                                              withRules:[NSString stringWithFormat:@"Rules: \n- Your swimming or flounder speed equals to the level of this skill minus level of the task. Minimum speed is equal to character Physique level. \n- To keep afloat perform Swim check against the level of the task. Failing mean you submerge into water and began to drawn. To break surface character need to make number of successful checks equal to number failed checks. While drawing character can still move with minimal speed, but every turn should perform d8 Control checks against the level of the task to stay calm.\n- For every %d points of encumbrance (counting equipment you are wearing) your swim level decreased by 1.\n- If you need to save someone who's drowning you should carry unlucky by yourself his/her encumbrance will be half of target Toughness multiplied by its bulk plus the half of it's equpment weight. Resulting encumbrance can vary depending on the nature of the water. \n- Being, at least, half submerged to perform physique related actions you need to pass swim test against the level of the task. \nYou can hold your breath up to your Physyque level rounds. After that you will suffer automatically 1 stress point each round.\n - Walking in a current character might fall down if fails standart fall test against level* of the current. If character moves against water current his movement lowers accordingly to the level of the flow. \n- Characters which using smashing or blunt type of weapon can't deal damage underwater.",3]
                                                      withRulesExamples:@"*Here are examples of water current and their stats:\nLevel 0. d4 test. Quiet water.\nLevel 1. d4 test. Calm water.\nLevel 2. d6 test. Rough water.\nLevel 4. d6 test. Stormy water.\nLevel 6. d6 test. Very rapid flow."
                                                        withDescription:@"Advanced skill. The ability to keep afloat and efficiently control your body in a water."
                                                          withSkillIcon:nil
    ...
    if (!_climb){
        ...
        if (!existingSkillsTemplateWithThisName || existingSkillsTemplateWithThisName.count==0 || self.shouldUpdate){
            skillTemplate = [SkillTemplate newSkillTemplateWithUniqName:climbName
                                                              withRules:[NSString stringWithFormat:@"Rules:\n- Character climbing speed equals to the skill minus level of the task. Minimum speed is equal to character Physique level.\n- To climb an obstacle perform Climb check against level of the obstacle. If character fail you start slipping or fall down (depending on a type of obstacle: climbing a pillar character will slip, but ceiling with handholds - will fall down). If climb skill check drops below zero you automatically fall down (even if character would normally slip).\n- In addition for every %d points of encumbrance (counting equipment you are wearing) your climb level decreased by 1.\n- To perform any other physique related actions you need to pass climb test agaist level of the task.",5]
                                                      withRulesExamples:@"*Here are examples of obsticles and their stats:\nLevel 2. d6 test. A slope too steep to walk up, or a knotted rope with a wall to brace against, ladder. Or living obstacle which move with speed of 1-2 or fighting in close combat.  \nLevel 4. d6 test. A surface with ledges to hold on to and stand on, such as a very rough wall or a ship’s rigging. Any surface with adequate handholds and footholds (natural or artificial), such as a very rough natural rock surface or a tree, or an unknotted rope, or pulling yourself up when dangling by your hands. Or living obstacle which move with speed of 3-5. \nLevel 6. d6 test. An uneven surface with some narrow handholds and footholds, such as a typical wall in a dungeon or ruins. Or living obstacle which move with speed of 6-9.\nLevel 8. d6 test. A rough surface, such as a natural rock wall or a brick wall. Or an overhang or ceiling with handholds but no footholds. Or living obstacle which move with speed of 10 and more."
                                                        withDescription:@"Advanced skill. The ability to quickly climb slope too steep to walk, knotted rope etc. \n\n"
                                                          withSkillIcon:nil
                                                     withBasicXpBarrier:defaultAdvBasicBarrierLow
                                                   withSkillProgression:defaultAdvProgression
                                               withBasicSkillGrowthGoes:defaultAdvGrowhtGoesLow
    ...
    if (!_stealth){
        ...
        if (!existingSkillsTemplateWithThisName || existingSkillsTemplateWithThisName.count==0 || self.shouldUpdate){
            skillTemplate = [SkillTemplate newSkillTemplateWithUniqName:stealthName
                                                              withRules:[NSString stringWithFormat:@"Rules:\n- For every %d points of encumbrance (counting equipment you are wearing) your stealth level decreased by 1. \n- If you trying to  sneak up someone from behind by default you'll need to pass test depending on surroundings*. If you fail opponent can make perseption check to notice you. \n- If you trying to sneak past your opponent crossing his sight of vision you need to pass test* against opponent perseption.",defaultEncumbrancePenalties]
                                                      withRulesExamples:@"*Here are examples of surroundings and their stats:\n- d6 test. Normal surface/Dark room. \n- d12 test. Noisy surface(scree, shallow or deep bog, undergrowth, dense rubble)/Poorly-lit room. \n- d20 test. Very noisy(dense undergrowth, deep snow)/Well-lit room"
                                                        withDescription:@"Advanced skill. The ability to keep from being seen or heard, this skill combines hiding with being quiet. With level you gain understanding how move very quiet and pick up a matirials to make clothes to make as little sound as possible."
                                                          withSkillIcon:nil
                                                     withBasicXpBarrier:defaultAdvBasicBarrierLow
                                                   withSkillProgression:defaultAdvProgression
                                               withBasicSkillGrowthGoes:defaultAdvGrowhtGoesLow
    ...
    if (!_ride){
        ...
        if (!existingSkillsTemplateWithThisName || existingSkillsTemplateWithThisName.count==0 || self.shouldUpdate){
            skillTemplate = [SkillTemplate newSkillTemplateWithUniqName:rideName
                                                              withRules:[NSString stringWithFormat:@"Rules:\n- You should perform ride test on every ride related action against the level of the task*. All checks goes against the level of the task.\n- You can gain bonuses or penalties to the skill level depending on quality of saddle or pose you choose to ride the mount. A good saddle gives +2 to the skill level. Riding bareback gives -4 to riding. \n- For every %d points of encumbrance (counting equipment you are wearing) your ride level decreased by 1.\n- You should use Animal Handling skill Test Value for your mount for discipline tests if needed.",defaultEncumbrancePenalties]                                                      withRulesExamples:@"*Here are examples of tasks and their stats:\n- d6 test. Saddle, mount, ride, and dismount from a mount without a problem.\n- d6 test. Guide with knees. You can react instantly to guide your mount with your knees so that you can use both hands in combat. Make your Ride check at the start of your turn. If you fail, you can use only one hand this round because you need to use the other to control your mount. \n- d6 test. Stay in Saddle. You can react instantly to try to avoid falling when your mount rears or bolts unexpectedly or when you take damage. This usage does not take an action. \n- d8 test. Cover. You can react instantly to drop down and hang alongside your mount, using it as cover. You can’t attack or cast spells while using your mount as cover. If you fail your Ride check, you don’t get the cover benefit. This usage does not take an action. \n- d8 test. Soft fall. You can react instantly to try to take no damage when you fall off a mount — when it is killed or when it falls, for example. If you fail your Ride check, you take d6 falling damage. This usage does not take an action. \n- d8 test. Leap. You can get your mount to leap obstacles as part of its movement. If you fail your Ride check, you fall off the mount when it leaps and take the appropriate falling damage (at least d6 points). This usage does not take an action, but is part of the mount’s movement. \n- d8 test. Spur Mount. You can spur your mount to greater speed with an action. A successful Ride check increases the mount’s speed twice for 1 round but deals 1 fatigue to the creature. \n- d10 test. Control mount in battle. As an action, you can attempt to control a light horse, pony, heavy horse, or other mount not trained for combat riding while in battle. If you fail the Ride check, you can do nothing else in that round. You do not need to roll for warhorses or warponies. \n- d10 test. Fast mount or dismount. You can attempt to mount or dismount from a mount of up to one size category larger than yourself as a free action, provided that you still have an action available that round. If you fail the Ride check, mounting or dismounting is an action. You can’t use fast mount or dismount on a mount more than one size category larger than yourself. \n- d12 test. Stand on mount. This allows you to stand on your mount’s back even during movement or combat. You take no penalties to actions while doing so. \n- d20 test. Unconscious Control. As a free action, you can attempt to control a light horse, pony, or heavy horse while in combat. If the character fails, you control the mount with action point. You do not need to roll for warhorses or warponies. \n- d20 test. Attack from Cover. You can react instantly to drop down and hang alongside your mount, using it as one-half cover. You can attack and cast spells while using your mount as cover without penalty. If you fail, you don’t get the cover benefit."
    ...
    if (!_knavery){
        ...
        if (!existingSkillsTemplateWithThisName || existingSkillsTemplateWithThisName.count==0 || self.shouldUpdate){
            skillTemplate = [SkillTemplate newSkillTemplateWithUniqName:knaveryName
                                                              withRules:[NSString stringWithFormat:@"Rules:\n- You should perform knavery test on every knavery related action against the level of task*.\n- For every %d points of encumbrance (counting equipment you are wearing) your knavery level decreased by 1.\n- If you are trying to perform skill while someone watching you closely - to remain unnoticed you should pass knavery test against opponent perception.",defaultEncumbrancePenalties]
                                                      withRulesExamples:@"*Here are examples of tasks and their stats:\nLevel 2. d6 test. Palm a coin-sized object, make a coin disappear. \nLevel 4. d6 test. Lift a small object from a person. \nLevel 6. d12 test. Lift a sheathed weapon from another creature and hide it on the character’s person, if the weapon is no more than one size category larger than the character’s own size. \nLevel 8. d12 test. Make an adjacent, willing creature or object of the character’s size or smaller “disappear” while in plain view. In fact, the willing creature or object is displaced up to 10 feet away—make a separate knavery test to determine how well the “disappeared” creature or object is hidden."
                                                        withDescription:@"Advanced skill. Pick pockets, draw hidden weapons, show a trick. Your training allows you to pick pockets, draw hidden weapons, and take a variety of actions without being noticed."
                                                          withSkillIcon:nil
                                                     withBasicXpBarrier:defaultAdvBasicBarrierLow
                                                   withSkillProgression:defaultAdvProgression
                                               withBasicSkillGrowthGoes:defaultAdvGrowhtGoesLow
    ...
    if (!_senseMotive){
        ...
        if (!existingSkillsTemplateWithThisName || existingSkillsTemplateWithThisName.count==0 || self.shouldUpdate){
            skillTemplate = [SkillTemplate newSkillTemplateWithUniqName:senseMotiveName
                                                      ...
                                                      withRulesExamples:@"\nd8 test. Discern Secret Message.\nd12 test. Sence Enchantment."
    ...
    if (!_appraise){
        ...
        if (!existingSkillsTemplateWithThisName || existingSkillsTemplateWithThisName.count==0 || self.shouldUpdate){
            skillTemplate = [SkillTemplate newSkillTemplateWithUniqName:appraiseName
                                                      ...
                                                      withRulesExamples:@"*Here are examples of tasks and their stats:\nd6 test against rarity level of item. On success get item price. On fail price modified accordingly to value of the the dice multiplied by item rarity level. "
    ...
    if (!_hackDevice){
        ...
        if (!existingSkillsTemplateWithThisName || existingSkillsTemplateWithThisName.count==0 || self.shouldUpdate){
            skillTemplate = [SkillTemplate newSkillTemplateWithUniqName:hackDeviceName
                                                      ...
                                                      withRulesExamples:@"Warning! All checks goes against the level of the tasks presented here. \n*Here are examples of tasks and their stats:\nLevel 4. d6 test. Jam a lock. \nLevel 6. d6 test. Sabotage a wagon wheel. Lockpick a simple constructed lock. \nLevel 8. d8 test. Disarm a trap, reset a trap. Lockpick a average constructed lock. \nLevel 10. d10 test. Disarm a complex trap, cleverly sabotage a clockwork device. Lockpick a good constructed lock. \nLevel 12. d12 test. Sabotage ingeniously crafted mechanism. Lockpick an amazingly constructed or very exotic lock."
                                                        withDescription:@"Advanced skill. You are skilled at disarming traps and opening locks. In addition, this skill lets you sabotage simple mechanical devices, such as catapults, wagon wheels, and doors."
                                                          withSkillIcon:nil
                                                     withBasicXpBarrier:defaultAdvBasicBarrierLow
                                                   withSkillProgression:defaultAdvProgression
                                               withBasicSkillGrowthGoes:defaultAdvGrowhtGoesLow
    ...
    if (!_heal){
        ...
        if (!existingSkillsTemplateWithThisName || existingSkillsTemplateWithThisName.count==0 || self.shouldUpdate){
            skillTemplate = [SkillTemplate newSkillTemplateWithUniqName:healName
                                                      ...
                                                      withRulesExamples:@"Here are examples of tasks and their stats:\nd8 test. Treat Poison. Treat Disease. Treat Injuries. Long-Term Care..."
